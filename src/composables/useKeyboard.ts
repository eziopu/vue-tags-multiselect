...
import { APP_KEYDOWN_LOCK_BY_TAG_INPUT_FOCUS } from '../timeouts'
import { appHandleKeydownKey } from '../injectionKeys'

export default function useKeyboard(props: Record<string, any>, dep: Record<string, any>) {
  // ============== DEPENDENCIES ==============
  ...
  watch(
    ...
    async (value: number) => {
      ...
      displayOptions.forEach((option: Element) => {
  ...
  watch(
    ...
    async (value: number) => {
      ...
      if (value != -1) {
        (tagValues[tagValues.length - 1 - value] as HTMLElement).click()
      } else {
  ...
  watch(editTagIndex, async (value: number) => {
    if (value == -1) {
      return
    }
    ...
    for (let index = 0; index < tagValues.length; index++) {
      ...
      if (
        Number((tagValues[index] as HTMLElement).dataset.index) == value &&
    ...
    setTimeout(() => {
      if (isActiveElementContainApp() == true) {
        const actElm = document.activeElement as HTMLElement
    ...
    switch (event.key) {
      case 'Enter':
        ...
        if (keydown.verticalIndex != -1) {
          const displayOptions = getDisplayOptionElms()
          const option = displayOptions[keydown.verticalIndex] as HTMLElement
        ...
        try {
          ...
          if (isExecuted == false && tagsGroupByTitle.length != 0) {
            if (props.merge == true) {
              ...
              ].values.map((value: { index: number }) => value.index)
              ...
              const getLastTag = () => {
                const clearTags = tags.filter(
                  (tag: unknown) => tag != null && tag != undefined
                )
  ...
  const isHoverSystemOption = (): boolean => {
    return (
  ...
  // ============== PROVIDE ==============
  provide(appHandleKeydownKey, handleKeydown)
